<script>
  ...
  onMount(() => {
    const cursorWrapper = document.querySelector(".cursor-wrapper"); //wrapper
    ...
    if (getComputedStyle(cursorWrapper).display !== "none") {
      initiateCursor();
    }

    window.addEventListener("resize", initiateCursor());
  });
</script>
